...
class CarControllerParams:
  ...
  def __init__(self, CP):
    ...
    if CP.carFingerprint in (CAR.RANGEROVER_EVOQUE_2021):
...
class LandroverFlexrayPlatformConfig(PlatformConfig):
  dbc_dict: DbcDict = field(default_factory=lambda: {
    Bus.radar: "rangerover_evoque_2021" })
  wmis: set[WMI] = field(default_factory=set)
  lines: set[ModelLine] = field(default_factory=set)
  years: set[ModelYear] = field(default_factory=set)
...
class CAR(Platforms):
  RANGEROVER_EVOQUE_2021 = LandroverFlexrayPlatformConfig(
    [
...
EVA2_CARS = {CAR.LANDROVER_DEFENDER_2023, CAR.RANGEROVER_EVOQUE_2021}
