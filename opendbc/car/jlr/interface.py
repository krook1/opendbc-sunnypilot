...
from opendbc.car.jlr.carcontroller import CarController
from opendbc.car.jlr.carstate import CarState
...
class CarInterface(CarInterfaceBase):
  ...
  def _get_params(ret: structs.CarParams, candidate, fingerprint, car_fw, alpha_long, is_release, docs) -> structs.CarParams:
    ...
    # panda[0] -> NO_OUTPUT, panda[1] -> ALL_OUTPUT (temporary until JLR safety is ready)
    ret.safetyConfigs = [
  ...
  def _get_params_sp(stock_cp: structs.CarParams, ret: structs.CarParamsSP, candidate, fingerprint: dict[int, dict[int, int]],
    ...
    # Ensure JLR returns a valid CarParamsSP (even if no brand-specific tweaks yet)
    return ret
